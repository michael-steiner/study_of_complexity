...
class ParticleZero(Atom):
    ...
    def check_for_interactions(self, other):
        zero_interaction = False
        if other.type_of_particle == TypeOfParticle.particle_zero:
            if np.random.randint(0, 100000) <= 100:
...
class Universe:
    ...
    def move_zero_particles(self):
        ...
        for particle_type_zero_move_counter in range(len(self.zero_particles)):
            zero_particle_to_move = self.zero_particles.pop()
            zero_particle_to_move.simple_random_motion()
            for zero_particle_to_check_proximity in self.zero_particles:
                if zero_particle_to_move.check_distance(zero_particle_to_check_proximity) < 10:
                    if zero_particle_to_move.check_for_interactions(zero_particle_to_check_proximity):
                        set_of_particle_x_change_to_particle_y.add(zero_particle_to_move.numeric_label)
                        set_of_particle_x_change_to_particle_y.add(zero_particle_to_check_proximity.numeric_label)
            self.zero_particles.appendleft(zero_particle_to_move)

        for zero_particle_interaction_counter in range(len(self.zero_particles)):
            zero_particle_to_categorize = self.zero_particles.pop()
            if zero_particle_to_categorize.numeric_label in set_of_particle_x_change_to_particle_y:
                self.one_particles.append(zero_particle_to_categorize)
            else:
                self.zero_particles.appendleft(zero_particle_to_categorize)
